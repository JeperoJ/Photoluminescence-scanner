...
def subtract(source1,source2,calpath,rotation=0,speed=4000,drift=0.0466,nsteps=100,FPS=50):
    ...
    imgdiff_stretched = ingaas_processing.lin_stretch_img(imgdiff, 40, 99.99)
...
def savevideo(source,calpath,rotation=0,speed=4000,drift=0.02448,nsteps=100,FPS=50):
    ...
    for i in range(len(imagerot)):
        ...
        frame = ingaas_processing.lin_stretch_img(imagerot[i], 40, 99.99).astype(np.uint8)
...
def main():
    ...
    calpath=os.path.join(cwd,"Software/calibration")
    print(calpath)
    ...
    path = "C:/Users/jeppe/Desktop/Work Stuffs/LIPI-scans/scan_cont_2025-11-20_11-59.tiff"    
...
def loadSnapshot():
    ...
    K,P,DIM= ProcessInGaAs.loadCal(calpath, width, height)
    imgUndistorted= ingaas_processing.undistort(im, K, P, DIM)
...
def histogramPlot():
    ...
    images_lin = ingaas_processing.lin_stretch_img(images[400:450], 0.01, 99)
    print("Histogram")
